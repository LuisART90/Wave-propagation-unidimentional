...
% Se genera una matriz de conectividad
...
% a=C_Global(1,1);
% b=C_Global(end,end);
% C_Global(1,1)=a+(Propiedades_Generales(1,2));
% C_Global(end,end)=b+(Propiedades_Generales(1,2));
...
% axis([-10 length(M_Global)+10 -10 10])
% title('Velocidad')
% grid on
% 
% subplot(3,1,3)
...
% a_Homogeneo=C_Global_Homogeneo(1,1);
% b_Homogeneo=C_Global_Homogeneo(end,end);
% C_Global_Homogeneo(1,1)=a_Homogeneo+(a_Homogeneo*(1+Fraccion));
% C_Global_Homogeneo(end,end)=b_Homogeneo+(b_Homogeneo*(1+Fraccion));
...
%     figure(i)   
%     subplot(2,1,1)
%     plot(Aceleraciones(:,150*i))
%     grid on
% 
%     subplot(2,1,2)
%     plot(Aceleraciones_Homogeneo(:,150*i))
%     grid on
% end 
...
% figure
% for i=1:s
%     plot(Velocidades(:,i),'--r','DisplayName','Problema original')
%     hold on 
%     plot(Velocidades_Homogeneo(:,i),'--b','DisplayName','Problema auxiliar')
%     hold off
%     grid on
%     title('Comparación de desplazamientos')
%     axis([-10 No_nodos+10 -4 4])
%     legend
%     drawnow
%     % pause(0.1)
% end 
...
% figure
% for i=1:s
%     plot(Fuerzas_equivalentes(:,i))
%     grid on
%     axis([-10 No_nodos+10 -3 3])
%     title(['Paso de tiempo: ' num2str(i)])
%     drawnow
% end 
...
for i=2:s-1
    Desplazamientos_reducido(:,i+1)=Matriz_inversa*((dt^2)*((Fuerzas_equivalentes(:,i+1)-(K_Global*Desplazamientos_reducido(:,i))))+((dt)*C_Global*Desplazamientos_reducido(:,i))+(M_Global*((2*Desplazamientos_reducido(:,i))-Desplazamientos_reducido(:,i-1))));
end 
...
for i=1:s
    subplot(2,1,1)
    plot(Desplazamientos_reducido(:,i),'-r')
    axis([-20 No_nodos+20 -1.5 1.5])
    title('Problema Auxiliar (Homogéneo)')
    title('Problema \Delta')
    grid on
...
Desplazamientos(2:end,1:s)=Desplazamiento_original;
...
for i=1:s
subplot(2,1,1)
plot(Coordenadas_nodales(2:end,:),Desplazamiento(:,i),'-k')
% hold on
% plot(Coordenadas_nodales,Desplazamientos(:,i),'xr')
% hold off
title('Problema original')
axis([-1 Longitud+1 -2 2])
grid on
